...
const AllKycUsers = (props: Details) => {
  const [list, setList] = useState(props?.kycs);
  ...
  const handleUpdate = async (item: string, e: any) => {
    ...
    let updated = await fetch(`${process.env.NEXT_PUBLIC_APIURL}/kyc/kycstatus`, {
      headers: {
  ...
  return (
        ...
        <table width="100%">
          ...
          <tbody>
            {list?.map((item: any, index: number) => {
              ...
              if (active === 2 && item.isVerified === false) {
                return;
              } else if (active === 3 && item.isReject === false) {
                return;
              }
              else if(active === 1 && (item.isReject === true || item.isVerified === true)){
                return
              } 
                ...
                >
                  ...
                  <td className="admin-table-data">
                    <div className="flex gap-[5px] items-center">
                      <div
                        className={`w-[7px] h-[7px] mr-[5px] rounded-full ${item?.isVerified === true
                      ...
                      ></div>
                      <p
                        className={`text-[13px] font-public-sans font-normal leading-5 ${item?.isVerified === true
                  ...
                  {active === 1 &&
                    <td className="w-[20%]">
                      <div className="inline-flex items-center gap-10">
                        <button className="admin-outline-button !px-[10px] !py-[4px] whitespace-nowrap	" onClick={(e) => { handleUpdate(item?.user_id, e) }}>
                          Approve
                        </button>
                        <button className="admin-outline-button !text-[#F44336] !border-[#f443361f] !px-[10px] !py-[4px] whitespace-nowrap" onClick={(e) => { handleUpdate(item?.user_id, e) }}>
                          Reject
                        </button>
                      </div>
                    </td>
    ...
    {
      show && 
      <DocumentsModal show={show} setShow={setShow} data={data}/>
    }
    </>
  );
};
