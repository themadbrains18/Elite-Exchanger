...
const schema = yup.object().shape({
  ...
  usdt_min_trade: yup.number().positive("USDT minimum must be greater than '0'.").required("Please enter usdt minimum trade limit.").typeError("Please enter usdt minimum trade limit."),
});


const FutureEditPair = (props: ActiveSession) => {
  ...
  useEffect(() => {
    setValue("coin_id", props?.editPair?.coin_id);
    setValue("usdt_id", props?.editPair?.usdt_id);
    setValue("coin_symbol", props?.editPair?.coin_symbol);
    setValue("usdt_symbol", props?.editPair?.usdt_symbol);
    setValue("coin_fee",props?.editPair?.coin_fee);
    setValue("usdt_fee",props?.editPair?.usdt_fee);
    setValue("coin_min_trade",props?.editPair?.coin_min_trade);
    setValue("usdt_min_trade",props?.editPair?.usdt_min_trade);
  }, [])
