...
router
    .get(async (req, res) => {
        try {
            ...
            let data = await getMethod(`${process.env.NEXT_PUBLIC_APIURL}/staking/getbytoken/${req.query.tokenid}/${req.query.userid}`, token);
...
router
    .post(async (req, res) => {
        try {
            ...
            let token = req.headers.authorization;
            // Call the API using a helper function and pass the necessary parameters.
            let data = await postData(`${process.env.NEXT_PUBLIC_APIURL}/staking/create`, JSON.parse(formData), token);
            // Respond with a 200 status and send the retrieved data.
            return res.status(200).send({ data });
