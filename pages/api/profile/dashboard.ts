...
router
    .post(async (req: NextApiRequest, res: NextApiResponse) => {
        try {
            ...
            let data = await postData(`${process.env.NEXT_PUBLIC_APIURL}/user/profile/create`, JSON.parse(formData), token);
...
router.get(async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        ...
        let data = await getMethod(`${process.env.NEXT_PUBLIC_APIURL}/user/profile`, token);
