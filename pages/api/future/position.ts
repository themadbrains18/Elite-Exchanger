    ...
    */
    async (req, res) => {
        try {
            ...
            let data = await getMethod(`${process.env.NEXT_PUBLIC_APIURL}/position/${req?.query?.userid}`, token);
    ...
    */
    async (req, res) => {
        try {
            ...
            let token = req.headers.authorization;
            let data = await postData(`${process.env.NEXT_PUBLIC_APIURL}/position/create`, JSON.parse(formData), token);
