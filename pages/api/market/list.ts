...
router
    .get(async (req, res) => {
        try {
            ...
            let data = await getMethod(`${process.env.NEXT_PUBLIC_APIURL}/market/admin/all/${itemOffset}/${itemsPerPage}`, token);
...
router
    .get(async (req, res) => {
        try {
            ...
            let token = req.headers.authorization;
            // Call the API using a helper function and pass the necessary parameters.
            let data = await getMethod(`${process.env.NEXT_PUBLIC_APIURL}/market/order/list/${user_id}/${itemOffset}/${itemsPerPage}`, token);
