...
router
    .get(async (req, res) => {
        try {
            ...
            let data = await getMethod(`${process.env.NEXT_PUBLIC_APIURL}/watchlist/${req.query.userid}`, token);
...
router
    .post(async (req, res) => {
        try {
            ...
            let token = req.headers.authorization;
            let data = await postData(`${process.env.NEXT_PUBLIC_APIURL}/watchlist/create`, JSON.parse(formData), token);
            return res.status(200).send({ data });
