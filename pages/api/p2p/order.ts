    ...
    */
    async (req: NextApiRequest, res: NextApiResponse) => {
        try {
            ...
            let data = await getMethod(`${process.env.NEXT_PUBLIC_APIURL}/p2p/order/${req.query.orderid}`, token);
    ...
    */
    async (req: NextApiRequest, res: NextApiResponse) => {
        try {
            ...
            let token = req.headers.authorization;
            let data = await postData(`${process.env.NEXT_PUBLIC_APIURL}/p2p/payment/method`, JSON.parse(formData), token);
            return res.status(200).send({ data });
        } catch (error: any) {
            throw new Error(error.message)
        }
    ...
    */
    async (req: NextApiRequest, res: NextApiResponse) => {
        try {
            ...
            let formData = AES.decrypt(decodedStr, `${process.env.NEXT_PUBLIC_SECRET_PASSPHRASE}`).toString(enc.Utf8);
            let token = req.headers.authorization;
            let data = await putData(`${process.env.NEXT_PUBLIC_APIURL}/p2p/cancel`, JSON.parse(formData), token);
            return res.status(200).send({ data });
        } catch (error: any) {
            throw new Error(error.message)
        }
