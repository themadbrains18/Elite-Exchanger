    ...
    */
    async (req, res) => {
        try {
            ...
            let data = await getMethod(`${process.env.NEXT_PUBLIC_APIURL}/chat/all/${req.query.orderid}`, token);
    ...
    */
    async (req: NextApiRequest, res: NextApiResponse) => {
        try {
            ...
            let token = req.headers.authorization;
            let data = await postData(`${process.env.NEXT_PUBLIC_APIURL}/chat/create`, JSON.parse(formData), token);
            return res.status(200).send({ data });
        } catch (error: any) {
            throw new Error(error.message)
        }
