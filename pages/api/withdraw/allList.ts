    ...
    */
    async (req, res) => {
        try {
            ...
            let data = await getMethod(`${process.env.NEXT_PUBLIC_APIURL}/withdraw/admin/withdrawListByLimit/${itemOffset}/${itemsPerPage}`, token);
...
router.get(
    ...
    */
    async (req, res) => {
        try {
            let { user_id, itemOffset, itemsPerPage, currency, date } = req.query;
            let token = req.headers.authorization;
            let data = await getMethod(`${process.env.NEXT_PUBLIC_APIURL}/withdraw/history/${user_id}/${itemOffset}/${itemsPerPage}/${currency}/${date}`, token);
            return res.status(200).send({ data });
        } catch (error: any) {
            throw new Error(error.message)
        }
