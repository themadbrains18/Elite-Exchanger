...
router.get(
    ...
    */
    async (req, res) => {
        try {
            const { itemOffset, itemsPerPage, currency, date } = req.query
            let token = req.headers.authorization;
            let data = await getMethod(`${process.env.NEXT_PUBLIC_APIURL}/withdraw/history/${req.query.user_id}/${itemOffset}/${itemsPerPage}/${currency}/${date}`, token);
