...
/**
 * RewardDetail component fetches and displays detailed reward information for a user.
 * It retrieves the reward ID from the URL, fetches the associated reward details, and displays them.
 * 
 * This component displays reward details such as:
 * - Reward ID
 * - Description
 * - Product type
 * - Trading pairs
 * - How to use the reward
 * - Claim and expiration timeline
 */
const RewardDetail = () => {
    ...
    /**
    * useEffect hook to fetch reward details whenever the reward ID changes.
    * It invokes the `getRewrdsDetails` function to fetch and set the reward data.
    */
    useEffect(() => {
        getRewrdsDetails()
    }, [rewards_Id]);
    ...
    /**
     * getRewrdsDetails is an asynchronous function that fetches the reward details
     * from the server using the user session information and reward ID.
     * It sets the state with the fetched data if successful.
     */
    const getRewrdsDetails = async () => {
        try {
            let rewardsDetail = await fetch(`${process.env.NEXT_PUBLIC_BASEURL}/rewards/detail?userid=${session?.user?.user_id}&rewardid=${rewards_Id}`, {
                method: "GET",
                headers: {
            ...
            if (rewardsDetail?.data) {
                setRewardDetail(rewardsDetail?.data);
            }
                        ...
                        <div className='max-w-[65%] w-full'>
                            <div className='flex items-center gap-[15px] justify-between'>
                                ...
                                <div className='flex items-center gap-[10px]'>
                                    <p className='info-14-18 dark:!text-white !text-black text-end max-w-[250px] w-full'>{reward?.id}</p>
                                    <svg
                            ...
                            <div className='flex items-center gap-[15px] justify-between mt-[20px]'>
                                ...
                                <p className='info-14-18 dark:!text-white !text-black text-end max-w-[250px] w-full'>{reward?.description}</p>
                            </div>
                            <div className='flex items-center gap-[15px] justify-between mt-[20px]'>
                        ...
                        <div className='max-w-[35%] w-full'>
                            <div className='rounded-[10px] bg-white'>
                                <div className='px-[24px] py-[30px] rounded-[10px] bg-primary-400 relative z-[1] group  after:w-[20px] after:h-[20px] after:absolute after:top-[calc(50%-10px)] after:left-[-10px]  after:bg-[#fff] after:dark:bg-d-bg-primary after:rounded-full before:w-[20px] before:h-[20px] before:absolute before:top-[calc(50%-10px)] before:right-[-10px] overflow-hidden before:bg-[#fff] before:dark:bg-d-bg-primary before:rounded-full'>
                                    <div className='flex items-center justify-center gap-[15px] relative'>
                                        <div className='text-center'>
                                            ...
                                            <p className='sm-text !text-white mt-[8px]'>{reward?.type}</p>
                                        </div>
                                ...
                                <div className='border border-[#e9edf2] pt-[40px] mt-[-40px] rounded-[10px] p-[24px]'>
                                    <button aria-label='for use of reward' type='button' className='mt-[15px] w-full solid-button !px-[20px] !py-[10px]' onClick={() => router.push({ pathname: '/future/BTCUSDT' })}>Use</button>
                                </div>
                            </div>
                        </div>
                    </div>
