...
const OrderBookFuture = (props: setHeight) => {
    ...
    useEffect(() => {
        const currentPrice = props?.currentToken?.token?.price || props?.currentToken?.global_token?.price;

        if (prevPrice !== null && currentPrice !== prevPrice) {
            setHighlow(currentPrice > prevPrice);
        }

        setPrevPrice(currentPrice);
    }, [props?.currentToken]);

    const currentPrice = props?.currentToken?.token !== null
        ? truncateNumber(props?.currentToken?.token?.price, 6)
        : truncateNumber(props?.currentToken?.global_token?.price, 6);
                    ...
                    <div className='bg-card-bg py-[6px] px-[20px] flex items-center justify-between dark:bg-omega bg-white my-[10px]'>
                        <div className='flex items-center gap-1'>
                            <svg
                                ...
                                xmlSpace="preserve"
                                xmlns="http://www.w3.org/2000/svg"
                                className={`w-3 ${highlow ? 'rotate-180' : ''}`}
                            ...
                            >
                                <path
                                    ...
                                    className={`${highlow ? 'fill-buy' : 'fill-sell'}`}
                                    fillRule="evenodd"
                                />
                                <g />
                                <g />
                            ...
                            <p className={`info-18 ${highlow ? 'text-buy' : 'text-sell'}`}>{currencyFormatter(truncateNumber(currentPrice, 6))}</p>
                        </div>
