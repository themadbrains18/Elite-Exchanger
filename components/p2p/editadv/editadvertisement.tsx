...
const EditAdverstisement = (props: propsData) => {
    ...
    return (
        <>
                                        ...
                                        {assets?.map((item: any, index: any) => {
                                            return (
                                                <div key={index} className="flex items-center md:mr-4 md:py-2 md:px-[18px] cursor-pointer">
                                                    <input id={`radio${item.id}`} {...register('token_id')} disabled onChange={() => selectToken(item)} type="radio" value={item.id} name="token_id" className="w-5 h-5 hidden bg-red-400 border-[transparent] focus:ring-primary dark:focus:ring-primary dark:ring-offset-primary  dark:bg-[transparent] dark:border-[transparent]" />
                                                    <label
            ...
            {step === 2 && <EditPaymentMethod step={step} setStep={setStep} setPaymentMethod={setPaymentMethod} masterPayMethod={props.masterPayMethod} userPaymentMethod={props.userPaymentMethod} selectedAssets={selectedAssets} assetsBalance={assetsBalance} price={step1Data.price} editPost={props.editPost}/>}
            {step === 3 && <EditResponse step={step} setStep={setStep} step1Data={step1Data} step2Data={step2Data} editPost={props.editPost}/>}
        </>
    );
};
