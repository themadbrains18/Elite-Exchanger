...
/**
 * Interface for the TradingPasswordAds component props
 * 
 * @interface TradingPasswordAdsProps
 */
interface TradingPasswordAdsProps {
    /**
   * Function to set the active state of the component/modal.
   * 
   * @param {boolean} active - Determines if the component is active or not.
   */
    setActive: Function,
    /**
   * Function to control the visibility of the component/modal.
   * 
   * @param {boolean} show - Boolean to show or hide the component/modal.
   */
    setShow: Function;
    /**
   * Function to submit the final ad data with the trading password.
   * 
   * @param {string} pass - The trading password used to submit the ad.
   */
    finalSubmitAds: Function;
    /**
   * Boolean indicating whether the component/modal is visible.
   * 
   * @type {boolean}
   */
    show: any;
    /**
  * Optional function to disable or enable a UI element, such as a submit button.
  * 
  * @param {boolean} disable - Boolean to disable or enable the UI element.
  */
    setDisable?: Function
    /**
   * Optional session data that can be used to verify the user's authentication status.
   * 
   * @type {any}
   */
    session?: any
...
const TradingPasswordAds = (props: TradingPasswordAdsProps) => {
    ...
    /**
     * Closes the popup by updating the parent component state.
     * This will hide the popup, set the active state to false, and disable the action buttons if applicable.
     */
    const closePopup = () => {
        // Set show state to false to hide the popup
        props?.setShow(false);
        // Set active state to false to deactivate the popup
        props.setActive(false);
        // If the setDisable function is passed, disable the UI element (like a button)
        props.setDisable && props.setDisable(false)
    }
    /**
     * Reference to the wrapper element used to detect outside clicks for closing the popup.
     * 
     * @type {React.RefObject<any>}
     */
    const wrapperRef: any = useRef(null);
    /**
     * Handles the logic for closing the popup when clicking outside of it.
     * This function is triggered by any click outside the wrapper element.
    */
    clickOutSidePopupClose({ wrapperRef, closePopup });

    /**
     * Effect to show the popup when the showForgetPopup state changes.
     * This will trigger the popup visibility change when the `showForgetPopup` value updates.
     */
    useEffect(() => {
        // Show the popup when `showForgetPopup` state changes
        props?.setShow(true)
    }, [showForgetPopup])
