...
const Verification = (props: propsData) => {
  ...
  const sendOtp = async () => {
    try {
      ...
      let userExist = await fetch(`${process.env.NEXT_PUBLIC_BASEURL}/user/${props.api}`, {
        method: "POST",
        headers: {
      ...
      <div className="flex gap-5 bg-[url('/assets/register/ellipsebg.svg')] bg-[length:75%]  bg-no-repeat lg:bg-none">
        ...
        <div className="max-w-[902px] w-full ">
          ...
          <div className="mt-0 lg:mt-[140px] lg:p-0 p-5  max-w-[calc(100%-30px)] md:mx-0 mx-auto md:mb-0 mb-[10px]  lg:bg-[transparent] lg:dark:bg-[transparent] bg-white lg:rounded-none rounded-10 dark:bg-d-bg-primary md:max-w-[562px] w-full">
            ...
            <p className="mb-5 text-center  lg:mb-[70px] md-text">Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry.</p>
            <div className="flex flex-col gap-[15px] lg:gap-5 mb-[30px]">
              ...
              <div className={`flex gap-5 items-center  w-full cursor-pointer bg-[transparent]`} >
                <input id={`custom-radio`} type="radio" value="" disabled={props.isEmail === true ? true : false} checked={props.isEmail === true ? false : true} name="colored-radio" className="hidden w-5 h-5 max-w-full   bg-red-400 border-[transparent] focus:ring-primary dark:focus:ring-primary dark:ring-offset-primary  dark:bg-[transparent] dark:border-[transparent]" />
              ...
              <input type="number" placeholder="Enter Phone Number " className="input-cta" disabled={true} value={props.isEmail === false ? props.formData.username : ''} />
            </div>
            <div className="flex flex-col gap-[15px] lg:gap-5">
              ...
              <div className={`flex gap-5 items-center  w-full cursor-pointer bg-[transparent]`} >
                <input id={`custom-radio2`} type="radio" value="" disabled={props.isEmail === false ? true : false} checked={props.isEmail === true ? true : false} name="colored-radio" className="hidden w-5 h-5 max-w-full   bg-red-400 border-[transparent] focus:ring-primary dark:focus:ring-primary dark:ring-offset-primary  dark:bg-[transparent] dark:border-[transparent]" />
              ...
              <input type="email" placeholder="Enter Email " className="input-cta" disabled={true} value={props.isEmail === true ? props.formData.username : ''} />
            </div>
