                        ...
                        {currentItems && currentItems?.length > 0 && currentItems?.map((item: any, index: any) => {
                            return (
                                <tr key={index} className=" dark:hover:bg-black-v-1  group rounded-5 hover:bg-[#FEF2F2] cursor-pointer" onClick={() => (item?.tradepair !== null ? router.push(`/chart/${item.symbol}`) : '')}>
                                    ...
                                    <td>
                                        <p className="info-14-18 !text-[14px] md:!text-[16px] dark:text-white  ">${new Intl.NumberFormat().format(item.price.toFixed(5))}</p>
                                    </td>
                                    <td className="max-[1023px]:hidden">
                                        <div className={` items-center gap-[10px] flex`}>
                                            <p className={`footer-text-secondary !text-[14px] md:!text-[16px] ${item.status == "high" ? "!text-[#03A66D]" : "!text-[#DC2626]"}`}>{new Intl.NumberFormat().format(item.circulatingSupply)}</p>
                                            <IconsComponent type={item.status} active={false} hover={false} />
                                        </div>
                                    </td>

                                    <td className="max-[1023px]:hidden">
                                        <p className="info-14-18 !text-[14px] md:!text-[16px] dark:text-white">${new Intl.NumberFormat().format(item.totalSupply)}</p>
                                    </td>
                                    <td className="max-[1023px]:hidden">
                                        <p className="info-14-18 !text-[14px] md:!text-[16px] dark:text-white">${new Intl.NumberFormat().format(item.maxSupply) || 0}</p>
                                    </td>
                                    <td className="max-[1023px]:hidden">
